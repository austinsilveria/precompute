...
from precompute import HookVariableNames, PrecomputeContext
...
class GPTNeoXAttention(nn.Module):
    ...
    def forward(
        ...
        query = pctx.hook_tensor(HookVariableNames.QUERY_STATES, query)
        key = pctx.hook_tensor(HookVariableNames.KEY_STATES, key)
        value = pctx.hook_tensor(HookVariableNames.VALUE_STATES, value)

        # Compute attention
        attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask)
        attn_weights = pctx.hook_tensor(HookVariableNames.ATTN_PROBS, attn_weights)
        attn_output = pctx.hook_tensor(HookVariableNames.ATTN_WEIGHTED_VALUES, attn_output)
...
class GPTNeoXMLP(nn.Module):
    ...
    def forward(self, hidden_states, pctx: Optional[PrecomputeContext] = None):
        hidden_states = self.dense_h_to_4h(hidden_states)
        hidden_states = pctx.hook_tensor(HookVariableNames.POST_FC1, hidden_states)
        hidden_states = self.act(hidden_states)
        hidden_states = pctx.hook_tensor(HookVariableNames.POST_ACT, hidden_states)
...
class GPTNeoXLayer(nn.Module):
    ...
    def forward(
        ...
        if self.use_parallel_residual:
            ...
            ln = self.post_attention_layernorm(hidden_states)
            ln = pctx.hook_tensor(HookVariableNames.PRE_MLP, ln)
            mlp_output = self.mlp(ln, pctx=pctx)
            mlp_output = self.post_mlp_dropout(mlp_output)
            mlp_output = pctx.hook_tensor(HookVariableNames.MLP_RESIDUAL, mlp_output)
            hidden_states = mlp_output + attn_output + hidden_states
        else:
            # pseudocode:
            # x = x + attn(ln1(x))
            # x = x + mlp(ln2(x))
            attn_output = self.post_attention_layernorm(attn_output + hidden_states)
            attn_output = pctx.hook_tensor(HookVariableNames.PRE_MLP, attn_output)
            mlp_output = self.mlp(attn_output, pctx=pctx)
            mlp_output = self.post_mlp_dropout(mlp_output)
            mlp_output = pctx.hook_tensor(HookVariableNames.MLP_RESIDUAL, mlp_output)
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = pctx.hook_tensor(HookVariableNames.POST_TOK_EMBEDDINGS, hidden_states)
...
class HookedGPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        lm_logits = pctx.hook_tensor(HookVariableNames.LOGITS, lm_logits)
        ...
        if labels is not None:
            ...
            loss = loss_fct(shift_logits.view(-1, self.config.vocab_size), labels.view(-1))
            shaped = loss.view(lm_logits.size(0), lm_logits.size(1) - 1)
            shaped = pctx.hook_tensor(HookVariableNames.UNREDUCED_LOSS, shaped)
